...
- Tenga en cuenta que las acciones (actions) no vienen predefinidas por defecto, debe realizarlas desde 0, todas deben pasar por el serializador correspondiente.
...
- Enviar una captura de los distintos endpoints funcionando en **Postman**.
- Debe sobreescribir el método `.to_representation()` que poseen los serializadores para no mostrar el campo `description` al listar objetos o instancias.
...
Debe utilizar un parámetro estático e ingresar el `id` o `pk` del usuario y devuelva los resultados correspondientes.
